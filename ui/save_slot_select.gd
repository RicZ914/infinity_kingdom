...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if key_event.keycode >= KEY_1 and key_event.keycode <= KEY_5:
			var slot_index := key_event.keycode - KEY_1
...
func _build_ui() -> void:
	...
	subtitle_label.text = _locale_text(
		"Choose an archive before selecting a family.",
		"先选择一个档案，再选择本轮回的家族。",
		"先選擇一個檔案，再選擇本輪迴的家族。"
	)
	...
	detail_label.text = _locale_text(
		"1-5 select archive. Empty archives create a new fixed-record save.",
		"按 1-5 选择档案。空档会创建新的固定记录存档。",
		"按 1-5 選擇檔案。空檔會建立新的固定記錄存檔。"
	)
...
func _refresh_slots() -> void:
	...
	for index in range(slot_buttons.size()):
		...
		button.text = "%d. %s  |  %s  |  %s %d  |  %s %d/%d  |  %s %s" % [
			index + 1,
			String(slot.get("save_name", _locale_text("Archive", "档案", "檔案"))),
			state_text,
			_locale_text("Gen", "第几代", "第幾代"),
			int(slot.get("generation_index", 1)),
